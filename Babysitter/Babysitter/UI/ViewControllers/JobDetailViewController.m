...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self judgeFavoriteStatus];
    [SVProgressHUD dismiss];
}

- (void)judgeFavoriteStatus{
    for (id jobObject in [AppDelegate sharedInstance].favoriteList) {
        NSInteger favID = [jobObject[@"id"] integerValue];
        NSInteger currentJobID = [_jobObject[@"id"] integerValue];
        
        if (currentJobID == favID){
            self.bFavorite = YES;
            break;
        }else{
            self.bFavorite = NO;
...
- (void)setupUI{
    ...
    self.locLabel.text = [self.jobObject objectForKey:@"location"];
//    [NSString stringWithFormat:@"%@, %@", [self.jobObject[@"parent"] objectForKey:@"city"],
//                          [self.jobObject[@"parent"] objectForKey:@"state"]];
