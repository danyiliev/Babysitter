...
- (void)viewWillAppear:(BOOL)animated{
    ...
    self.searchTxtField.text = [NSString stringWithFormat:@"%@-%@", latString, lonString];
...
- (void)getJobs{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/api/babysitter/%lu/jobs", BASIC_URL, userId];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    [manager GET:urlString
         ...
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             if (responseObject) {
                  ...
                  if ([responseObject[@"data"] isKindOfClass:[NSDictionary class]]) {
                      ...
                      filteredListContent = [NSMutableArray arrayWithObject:responseObject[@"data"]];
                  }else if ([responseObject[@"data"] isKindOfClass:[NSArray class]]){
                      self.jobList = responseObject[@"data"];
                      filteredListContent = [NSMutableArray arrayWithArray:self.jobList];
                  }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (filteredListContent && filteredListContent.count > 0){
        ...
        cell.locLabel.text = [jobObject[@"parent"] objectForKey:@"address"];
    }
